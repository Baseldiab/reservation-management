...
function UserSearchReservations() {
  ...
  const debouncedSetQuery = useCallback(
    (value: string) => {
      debounce((searchValue: string) => {
        queryClient.setQueryData(["my-reservations-search"], searchValue);
      }, 500)(value);
    },
    [queryClient]
  );
  ...
  const handleReset = useCallback(() => {
    setSearchValue("");
    queryClient.setQueryData(["my-reservations-search"], "");
  ...
  return (
    <div className="flex-1 min-w-[200px] self-start md:max-w-[400px] relative max-md:!w-full">
      <Input
        defaultValue={
          queryClient.getQueryData(["my-reservations-search"]) as string
        }
        value={searchValue}
