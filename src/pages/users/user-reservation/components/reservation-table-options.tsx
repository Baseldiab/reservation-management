...
import AddEditReservationDialog from "@/pages/users/user-reservation/components/add-edit-reservation";
...
export default function UserReservationTableOptions({
  ...
  const { data: filters } = useQuery<ReservationFilterParams>({
    queryKey: ["user-reservations-filters"],
  });
  const { data: searchValue } = useQuery<string>({
    queryKey: ["user-reservations-search"],
  });
  ...
  const deleteReservationMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.setQueryData(
        ["user-reservations", filters, searchValue],
